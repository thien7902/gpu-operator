...
func (p *Plugin) validateGPUResource() error {
	for retry := 1; retry <= gpuResourceDiscoveryWaitRetries; retry++ {
		...
		// disable plugin validation if vgpu is enabled
		vgpuLabel, exist := node.Labels["gpu"]
		if exist && vgpuLabel == "on" {
			log.Infof("vgpu is enabled on this node, skipping plugin validator")
			return nil
		}
